    ...
    static bool HandleReloadCreatureTemplateCommand(ChatHandler* handler, const char* args)
    {
        ...
        for (Tokens::const_iterator itr = entries.begin(); itr != entries.end(); ++itr)
        {
            ...
            for (uint8 i = 0; i < MAX_KILL_CREDIT; ++i)
                cInfo->KillCredit[i] = fields[1 + i].GetUInt32();

            cInfo->Modelid1          = fields[3].GetUInt32();
            cInfo->Modelid2          = fields[4].GetUInt32();
            cInfo->Modelid3          = fields[5].GetUInt32();
            cInfo->Modelid4          = fields[6].GetUInt32();
            cInfo->Name              = fields[7].GetString();
            cInfo->SubName           = fields[8].GetString();
            cInfo->IconName          = fields[9].GetString();
            cInfo->GossipMenuId      = fields[10].GetUInt32();
            cInfo->minlevel          = fields[11].GetUInt8();
            cInfo->maxlevel          = fields[12].GetUInt8();
            cInfo->faction_A         = uint32(fields[13].GetUInt16());
            cInfo->faction_H         = uint32(fields[14].GetUInt16());
            cInfo->npcflag           = fields[15].GetUInt32();
            cInfo->speed_walk        = fields[16].GetFloat();
            cInfo->speed_run         = fields[17].GetFloat();
            cInfo->scale             = fields[18].GetFloat();
            cInfo->rank              = uint32(fields[19].GetUInt8());
            cInfo->mindmg            = fields[20].GetFloat();
            cInfo->maxdmg            = fields[21].GetFloat();
            cInfo->dmgschool         = uint32(fields[22].GetInt8());
            cInfo->attackpower       = fields[23].GetUInt32();
            cInfo->dmg_multiplier    = fields[24].GetFloat();
            cInfo->baseattacktime    = fields[25].GetUInt32();
            cInfo->rangeattacktime   = fields[26].GetUInt32();
            cInfo->unit_class        = uint32(fields[27].GetUInt8());
            cInfo->unit_flags        = fields[28].GetUInt32();
            cInfo->dynamicflags      = fields[29].GetUInt32();
            cInfo->family            = uint32(fields[30].GetUInt8());
            cInfo->trainer_type      = uint32(fields[31].GetUInt8());
            cInfo->trainer_spell     = fields[32].GetUInt32();
            cInfo->trainer_class     = uint32(fields[33].GetUInt8());
            cInfo->trainer_race      = uint32(fields[34].GetUInt8());
            cInfo->minrangedmg       = fields[35].GetFloat();
            cInfo->maxrangedmg       = fields[36].GetFloat();
            cInfo->rangedattackpower = uint32(fields[37].GetUInt16());
            cInfo->type              = uint32(fields[38].GetUInt8());
            ...
            for (uint8 i = SPELL_SCHOOL_HOLY; i < MAX_SPELL_SCHOOL; ++i)
                cInfo->resistance[i] = fields[43 + i -1].GetInt16();

            for (uint8 i = 0; i < CREATURE_MAX_SPELLS; ++i)
                cInfo->spells[i] = fields[49 + i].GetUInt32();
            ...
            cInfo->MovementType   = uint32(fields[61].GetUInt8());
            cInfo->InhabitType    = uint32(fields[62].GetUInt8());
            ...
            for (uint8 i = 0; i < MAX_CREATURE_QUEST_ITEMS; ++i)
                cInfo->questItems[i] = fields[68 + i].GetUInt32();

            cInfo->movementId         = fields[74].GetUInt32();
            cInfo->RegenHealth        = fields[75].GetBool();
            cInfo->equipmentId        = fields[76].GetUInt32();
            cInfo->MechanicImmuneMask = fields[77].GetUInt32();
            cInfo->flags_extra        = fields[78].GetUInt32();
            cInfo->ScriptID           = sObjectMgr->GetScriptId(fields[79].GetCString());
